...
# Migrating from `#[path_param]`

The attribute has been removed. Replace it and the tuple struct with a declaration that names the URL parameter:
...
| `segment!(kind = CatchAll, rename = "path");` | `path_param!(*path);` when the handler reads decoded segments |

Visibility moves from the struct to the start of the declaration.

The generated type name now comes from the URL name. Rename references when an old struct name did not round-trip through Pascal case, such as `PostID`, which becomes `PostId`.

The `path_param::<T>(cx)` call shape remains unchanged; update `T` when the generated type name changes.
