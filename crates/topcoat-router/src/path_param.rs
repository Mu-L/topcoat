...
/// the `path_param!` macro.
#[doc(hidden)]
pub fn path_param_segments<'cx>(cx: &'cx Cx, name: &str) -> PathSegments<'cx> {
    match raw_path_params(cx).find(name) {
        Some(RawPathParamValue::CatchAll { segments, .. }) => PathSegments::new(segments),
...
pub struct PathSegments<'params> {
    inner: slice::Iter<'params, Box<str>>,
}

impl<'params> PathSegments<'params> {
...
impl<'params> Iterator for PathSegments<'params> {
...
mod tests {
    ...
    fn segments<'params>(
        ...
        name: &str,
    ) -> Option<PathSegments<'params>> {
        match params.find(name) {
            Some(RawPathParamValue::CatchAll { segments, .. }) => Some(PathSegments::new(segments)),
