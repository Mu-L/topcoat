...
/// # Request handling
///
/// The router selects the route before this layer runs. Inner layers and the
/// route see the rewritten URI, while outer layers keep their original context.
/// Captured path parameters still contain the values from route matching.
/// The layer does not rewrite response headers or URLs generated by the handler.
#[derive(Debug, Clone)]
...
impl StripPrefixLayer {
    ...
    ///
    /// The scope uses the same route-prefix rules as other [`Layer`]s, including
    /// group segments and parameter names. It selects routes by their declared
    /// paths. The stripping pattern matches the incoming URI independently.
    ///
    /// # Example
    ///
    /// ```rust
